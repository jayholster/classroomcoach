create or replace function public.commit_simulation_turn(
  _session_id uuid,
...
begin
  ...
  if not found then
    raise exception 'Rehearsal not found.' using errcode = 'P0002';
  end if;

  if _session.owner_id <> auth.uid() then
    raise exception 'You do not have permission to add to this rehearsal.' using errcode = '42501';
  end if;

  if _session.ended_at is not null then
    raise exception 'This rehearsal has already ended.' using errcode = 'P0001';
  end if;

  select coalesce(max(sequence), 0) + 1 into _next
  from public.simulation_events
  where session_id = _session_id;

  if _expected_sequence is not null and _expected_sequence <> _next then
    raise exception 'Another response was recorded first. Reload the rehearsal to see the latest turn.'
      using errcode = 'P0001';
  end if;
  ...
  insert into public.simulation_events (
    ...
    _session_id, _session.owner_id, _next, 'turn', _session.scenario_id, _session.scenario_version_id,
...
revoke all on function public.commit_simulation_turn(uuid, integer, text, jsonb, jsonb, jsonb, text, text, text, uuid, jsonb, text) from public, anon;
...
create unique index if not exists simulation_states_session_key on public.simulation_states (session_id);
create unique index if not exists simulation_events_session_sequence_key on public.simulation_events (session_id, sequence);