...
begin
  ...
  if _session.owner_id <> auth.uid() then raise exception 'You do not have permission to add to this rehearsal.' using errcode = '42501'; end if;
...
$$;
revoke all on function public.commit_simulation_event(uuid, integer, text, text, jsonb, jsonb, jsonb, text, text, text, uuid, jsonb, text) from public, anon;
grant execute on function public.commit_simulation_event(uuid, integer, text, text, jsonb, jsonb, jsonb, text, text, text, uuid, jsonb, text) to authenticated;